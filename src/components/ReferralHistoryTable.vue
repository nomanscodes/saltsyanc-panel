<template>
  <div
    class="bg-[var(--bg-primary)] rounded-lg border border-[var(--border-primary)] p-3 md:p-xl"
  >
    <div class="flex flex-col">
      <h2
        class="text-lg md:text-2xl text-[var(--text-primary)] font-semibold mb-3 md:mb-0"
      >
        My Referral History
      </h2>
      <div
        class="flex flex-col sm:flex-row flex-1 sm:flex-initial gap-2 md:gap-3 py-3 md:py-base"
      >
        <!-- Search Bar -->
    ...
    <!-- Loading State -->
    <div
      v-if="referralStore.loading && referralStore.referrals.length === 0"
      class="py-12"
    >
      <div class="flex flex-col items-center justify-center gap-4">
        <div
          class="w-12 h-12 border-4 border-[var(--border-primary)] border-t-[var(--bg-brand-solid)] rounded-full animate-spin"
        ></div>
    ...
    <div v-else-if="referralStore.isEmpty" class="py-12">
      <div class="flex flex-col items-center justify-center gap-4">
        <svg
          class="w-16 h-16 text-[var(--text-tertiary)]"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        <p class="text-[var(--text-secondary)] text-lg font-medium">
          No referrals found
        </p>
        <p class="text-[var(--text-tertiary)] text-sm">
          Start adding referrals to see them here
        </p>
      </div>
    </div>
    ...
    <div
      v-else
      class="overflow-x-auto rounded-lg border border-[var(--border-secondary)] -mx-3 md:mx-0"
    >
      <table class="min-w-full divide-y divide-gray-200 text-xs md:text-sm">
        <thead class="bg-[var(--bg-secondary)]">
          <tr>
            <th
              class="px-2 md:px-xl py-2 md:py-lg text-left font-semibold text-[var(--text-primary)] text-xs md:text-sm whitespace-nowrap"
            >
        ...
        <tbody class="divide-y divide-gray-100">
          <tr
            v-for="(item, index) in referralStore.referrals"
            :key="item.id"
            class="hover:bg-[var(--bg-primary-hover)] transition"
          >
            <td
              class="px-2 md:px-xl py-2 md:py-lg text-[var(--text-primary)] font-medium text-xs md:text-sm whitespace-nowrap"
            >
              <div class="flex items-center gap-1 md:gap-2">
                <input
                  type="checkbox"
            ...
            <td class="px-2 md:px-xl py-2 md:py-lg">
              <span
                class="px-1.5 md:px-2.5 py-0.5 md:py-1 rounded-full text-[10px] md:text-xs font-medium whitespace-nowrap"
                :class="{
    ...
    <div
      v-if="
        !referralStore.loading &&
        !referralStore.isEmpty &&
        !referralStore.hasError
      "
      class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs md:text-sm text-[var(--text-secondary)] mt-3 md:mt-4"
    >
      <p class="text-xs md:text-sm">
        {{ selectedRows.length }} of {{ referralStore.total }} row(s) selected.
      </p>
      <div class="flex flex-wrap items-center gap-2 text-xs md:text-sm">
        ...
        <div class="flex items-center gap-1">
          ...
          <span class="px-1 md:px-2 text-xs md:text-sm whitespace-nowrap"
            >Page {{ referralStore.currentPage }} of
            {{ referralStore.totalPages }}</span
          >
          <button
            class="px-2 py-1 border border-[var(--border-primary)] rounded disabled:opacity-40 hover:bg-[var(--bg-secondary)] transition-colors text-xs md:text-sm"
...
watch(
  ...
  () => {
    if (
      ...
      selectedRows.value.length < referralStore.referrals.length
    ) {
