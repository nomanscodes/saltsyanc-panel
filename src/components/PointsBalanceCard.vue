<template>
  <div class="flex flex-col gap-4 md:gap-6">
    ...
    <CardWrapper>
      <div
        class="py-3 px-4 md:py-base md:px-lg relative flex items-center gap-2 md:gap-3"
      >
        <div class="text-4xl md:text-[64px]">🏆</div>

        <div class="flex flex-1 flex-col gap-1 md:gap-2">
          <div class="text-[var(--text-quaternary)] text-sm md:text-xl">
            Your Points Balance
          </div>
          <div v-if="pointsStore.loading" class="flex items-center gap-2">
            <div
              class="w-24 md:w-32 h-8 md:h-10 bg-[var(--bg-secondary)] animate-pulse rounded"
            ></div>
          </div>
          <div
            v-else-if="pointsStore.hasError"
          ...
          <div v-else class="">
            <div class="flex items-center justify-between gap-2">
              <b class="text-[var(--text-primary)] text-2xl md:text-4xl">
                ...
                <span
                  class="text-[var(--text-quaternary)] text-lg md:text-2xl font-normal"
                >
    ...
    <div class="grid grid-cols-2 gap-3 md:gap-6">
      <CardWrapper
        v-for="item in pointsStore.pointsBreakdown"
        :key="item.label"
      >
        <div
          class="py-3 px-4 md:py-base md:px-lg relative flex items-center gap-2 md:gap-3"
        >
          ...
          <div class="flex flex-1 flex-col gap-1 md:gap-2">
            <div class="text-[var(--text-primary)] text-xs md:text-md">
              {{ item.label }}
            </div>
            <div
              v-if="pointsStore.loading"
              class="w-16 md:w-20 h-6 md:h-8 bg-[var(--bg-secondary)] animate-pulse rounded"
            ></div>
            <b v-else class="text-[var(--text-primary)] text-xl md:text-3xl">{{
              item.value
            }}</b>
          </div>
        </div>
